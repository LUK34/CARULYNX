...
def write_to_csv(clients, filename='scan_results.csv'):
...
def main(cidr):
   ...
   # print(f"Scanning {network.num_addresses} addresses in {cidr}...\n")
    ...
    with tqdm(total=len(list(network.hosts())), desc="Scanning Progress", ncols=100) as progress:
        ...
        for ip in network.hosts():
            ...
            thread = threading.Thread(target=scan, args=(str(ip), results_queue, progress))
            thread.start()
...
# python .\11_Network_Scanner_v2.py
# 192.168.1.0/24